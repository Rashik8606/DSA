...
def find_target_nums(arr,targetSum):
    empty_arr = []
    for i in range(len(arr)):
        current = arr[i]
        difference = targetSum - current

        if difference in empty_arr:
            return current, difference
        else:
            empty_arr.append(arr[i])
    return empty_arr
print(find_target_nums([6,5,4,3,9,8,0],10))


def count_arr_num(arr,targetNums):
    count_num = 0
    empty_arr = []
    for i in range(len(arr)):
        difference = targetNums - arr[i]
        for j in range(len(empty_arr)):
            if empty_arr[j] == difference:
                count_num += 1
        empty_arr.append(arr[i])
    return count_num
print(count_arr_num([1, 5, 7, -1, 5],6))



def find_sum_num(arr,targetNum):
    empty_arr = []

    for i in range(len(arr)):
        current = arr[i]
        difference = targetNum - current
